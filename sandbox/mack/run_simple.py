...
def train(logdir, env_id, num_timesteps, lr, timesteps_per_batch, seed, num_cpu, max_episode_len):
    ...
    logger.configure(logdir, format_strs=['stdout', 'json', 'tensorboard'])
...
@click.command()
@click.option('--logdir', type=click.STRING, default='../../ray_results/MACK_MPE_v2')
@click.option('--env', type=click.Choice(['simple', 'simple_speaker_listener',
                                          'simple_crypto', 'simple_push',
                                          'simple_tag', 'simple_spread', 'simple_adversary']), default='simple_adversary')
