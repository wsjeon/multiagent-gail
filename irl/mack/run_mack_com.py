...
from irl.mack.com_gail import learn
...
def train(logdir, env_id, num_timesteps, lr, timesteps_per_batch, seed, num_cpu, max_episode_len, expert_path,
          traj_limitation, ret_threshold, dis_lr, disc_type='competitive_single', bc_iters=500):
    def create_env(rank):
        def _thunk():
...
@click.option('--expert_path', type=click.STRING, default='/atlas/u/hyren/maexperts/simple_spread_two.pkl')
...
@click.option('--disc_type', type=click.Choice(['competitive_single']), default='competitive_single')
@click.option('--bc_iters', type=click.INT, default=500)
@click.option('--max_episode_len', type=click.INT, default=50)
def main(logdir, env, expert_path, atlas, seed,
         traj_limitation, ret_threshold, dis_lr, disc_type, bc_iters, max_episode_len):
    env_ids = [env]
    lrs = [0.1]
    seeds = [seed]
    batch_sizes = [1000]
    ...
    if atlas:
        ...
        logdir = '/atlas/u/hyren/exps'
    print('logging to: ' + logdir)

    if env == 'simple_spread':
        expert_path = '/atlas/u/tsong/exps/mack/simple_spread/l-0.1-b-1000/seed-4/checkpoint11000.pkl'
    elif env == 'simple_speaker_listener':
        expert_path = '/atlas/u/tsong/exps/mack/simple_speaker_listener/l-0.1-b-500/seed-2/checkpoint22000.pkl'
    elif env == 'simple_tag':
        expert_path = '/atlas/u/tsong/exps/mack/simple_tag/l-0.1-b-1000/seed-4/checkpoint11000.pkl'
    elif env == 'simple_push':
        expert_path = '/atlas/u/hyren/exps/mack/simple_push/l-0.1-b-1000/seed-1/checkpoint17500.pkl'

    for env_id, seed, lr, batch_size, max_episode_len in itertools.product(
            env_ids, seeds, lrs, batch_sizes, max_episode_lens):
        train(logdir + '/gail/' + env_id + '/' + disc_type + '/s-{}/l-{}-b-{}-d-{}-c-{}/seed-{}'.format(
              traj_limitation, lr, batch_size, dis_lr, bc_iters, seed),
              env_id, 8e6, lr, batch_size, seed, 4, max_episode_len, expert_path,
